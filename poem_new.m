function [ph_cmb,mag_cmb,offset_real, offset_imaginary] = poem_new(mag, pha, vox, te, mask, smooth_method, parpool_flag)
...
nii = make_nii(angle(offsets),vox);
save_nii(nii,'offsets_smooth_angle.nii');

nii = make_nii(offsets,vox);
save_nii(nii,'offsets_smooth.nii');

tmp=zeros(imsize(1),imsize(2),imsize(3),nrcvrs);
for chan = 1:nrcvrs
    tmp(:,:,:,chan) = offsets(:,:,:,1,chan);
    
%     tmp_2(:,:,:) = offsets(:,:,:,1,chan);
%     
%     a=real(tmp_2);
%     b=imag(tmp_2);
% 
% 
%     nii = make_nii(a,vox);
%     real_part = strcat('offsets_smooth_real_cH', chan,'.nii');
%     save_nii(nii,real_part);
% 
%     nii = make_nii(b,vox);
%     save_nii(nii,'offsets_smooth_imaginary.nii');

end
% nii = make_nii(tmp,vox);
% save_nii(nii,'offsets_smooth_4D.nii');
% 
% nii = make_nii(angle(tmp),vox);
% save_nii(nii,'offsets_smooth_4D_angle.nii');

offset_real=real(tmp);
offset_imaginary=imag(tmp);

% nii = make_nii(offset_real,vox);
% save_nii(nii,'offsets_smooth_real.nii');
% 
% nii = make_nii(offset_imaginary,vox);
% save_nii(nii,'offsets_smooth_imaginary.nii');
